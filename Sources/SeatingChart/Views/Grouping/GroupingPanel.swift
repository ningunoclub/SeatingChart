...
/// Floating panel for the groups. Same deal as the timer's and the name
/// picker's: it has to sit above a running PowerPoint slideshow on a second
/// monitor and never take the keyboard away from it.
final class GroupingPanel: NSPanel {
...
final class GroupingOverlayController {
    static let shared = GroupingOverlayController()

    /// Must differ from the other two overlays' — windows sharing one autosave
    /// name fight over the same saved frame.
    private static let autosaveName = "GroupingOverlay"
    /// Bigger than the other two: this one shows a grid, not a single line.
    private static let defaultSize = NSSize(width: 520, height: 320)

    private var panel: GroupingPanel?
    ...
    func show(store: AppStore) {
        ...
        store.grouping.isOverlayVisible = true
    }

    func hide(store: AppStore) {
        panel?.orderOut(nil)
        store.grouping.isOverlayVisible = false
    }
    ...
    private func makePanel(store: AppStore) -> GroupingPanel {
        let panel = GroupingPanel(
        ...
        panel.title = L("grouping.overlay_window")
        ...
        let host = NSHostingView(rootView: GroupingOverlayView().environment(store))
    ...
    /// Below the timer's corner, which is itself below the name picker's, so
    /// opening all three for the first time does not stack them on each other.
    private func positionTopRight(_ panel: NSPanel) {
        ...
        let y = max(visible.minY + 24, visible.maxY - size.height - 24 - 600)
